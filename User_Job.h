...
public:
	...
	//4 int job_size; //1 short, 2 medium, 3 large, 4 gpu, 5 huge
	//5 int job_status;// 0 means waiting, 1 means running, 2 means done, 3 means failure, 
	//6 int job_time;  //the time of total job needs
	//7 int run_time; //the time of job implement
	//8 int left_time;
	//9 int wait_time;
	//10 int node_id; //connect with the node  //如果能解决多个node可以识别，就改下 不用遍历nodes
